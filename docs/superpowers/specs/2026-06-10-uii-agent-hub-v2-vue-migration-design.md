...
| Midscene + Playwright UI 自动化 | ⏸ 可选最小落地 | 开放问题 ③ |
...
### 6.2 阶段化
- **阶段 1（现在，源码留本地）**：`pnpm deploy:pages` 本地命令完成 构建→推送产物仓库（git push，需要产物仓库推送权限）。
- **阶段 2（源码推上私有仓库后）**：预置的 `deploy.yml` 即刻生效（`peaceiris/actions-gh-pages` + `external_repository` + `DEPLOY_TOKEN` secret），push 即自动部署，零改动切换。
...
- **CI 门禁**：lint + test 全绿才构建/部署。
- **（开放问题 ③）** Playwright 最小 smoke（每页可达 + 关键元素存在），对齐规范 UI 自动化方向。
...
| # | 问题 | 我的建议 |
|---|---|---|
| ① | TDesign 本期是否引入 | **否**——纯展示站自有视觉已完备；控制台真实化时按规范引入并做 token 映射 |
| ② | 公开产物仓库的账号/仓库名（决定 Pages URL 与 base） | 待你提供，如 `<org>/uii-agent-hub-site`；执行部署任务前给到即可 |
| ③ | Playwright smoke 是否纳入本期 | **纳入最小集**（约 8 个页面可达性 spec，成本低、对齐规范） |
| ④ | 版本变更的触发约定 | 建议 **push tag `v*`** 触发正式部署，master push 仅构建验证 |
