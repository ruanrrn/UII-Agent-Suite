# UII Agent Suite v2 · Vue 工程化迁移设计（框架 + Mock 体系 + CI/CD）

- **日期**：2026-06-10
- **状态**：设计草案，待评审
- **前置**：v0 设计 `2026-06-04-uii-agent-suite-portal-design.md`（vanilla 原型已完成并合入 master，17/17 测试通过）
...
[GitHub Actions]（workflow 预置在源码仓库）
   ...
   └─▶ 上传 uii-agent-suite-mock-<version>.zip artifact（可直接下载部署）
```
...
| ② | 公开产物仓库的账号/仓库名 | **待用户提供**（如 `ruanrrn/UII-Agent-Suite`）；执行部署任务前给到即可，不阻塞迁移开发 |
